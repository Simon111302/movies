/**
 * Ad Blocker Utility
 * Blocks ads, ad scripts, and ad-related content
 */
...
function isAdElement(element: Element): boolean {
  ...
  const isWhitelisted = WHITELIST_SELECTORS.some(whitelist => 
    className.includes(whitelist) || id.includes(whitelist)
  );
  ...
  const hasAdPattern = specificAdPatterns.some(pattern => 
...
function isWhitelistedElement(element: Element): boolean {
  const className = element.className?.toString().toLowerCase() || '';
  const id = element.id?.toLowerCase() || '';
  return WHITELIST_SELECTORS.some(whitelist => 
    className.includes(whitelist) || id.includes(whitelist)
  );
}
